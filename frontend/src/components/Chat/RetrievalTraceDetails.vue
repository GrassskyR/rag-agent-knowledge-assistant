  ...
  <div v-if="msg.ragTrace" class="message-meta">
    <details class="reasoning-details">
      ...
      <div class="reasoning-content">
        ...
        <div v-if="msg.ragTrace.rag_fallback_reason" class="trace-line">
          Web fallback 判定：{{ msg.ragTrace.rag_fallback_reason }}
        </div>
        <div v-if="msg.ragTrace.web_search_used" class="trace-line">
          联网搜索：已启用
          <span v-if="msg.ragTrace.web_result_count !== null && msg.ragTrace.web_result_count !== undefined">
            （{{ msg.ragTrace.web_result_count }} 条结果）
          </span>
        </div>
        <div v-if="msg.ragTrace.tool_query" class="trace-line">
          工具查询：{{ msg.ragTrace.tool_query }}
        </div>
        <div v-if="msg.ragTrace.web_query" class="trace-line">
          联网查询：{{ msg.ragTrace.web_query }}
        </div>
        <div v-if="msg.ragTrace.web_engines && msg.ragTrace.web_engines.length" class="trace-line">
          搜索引擎：{{ msg.ragTrace.web_engines.join(', ') }}
        </div>
        <div v-if="msg.ragTrace.web_fetch_decision" class="trace-line">
          网页正文获取：{{ msg.ragTrace.web_fetch_decision }}
        </div>
        <div v-if="msg.ragTrace.web_fetch_count !== null && msg.ragTrace.web_fetch_count !== undefined" class="trace-line">
          已读取网页数：{{ msg.ragTrace.web_fetch_count }}
        </div>
        ...
        <div v-if="msg.ragTrace.web_sources && msg.ragTrace.web_sources.length" class="sources">
          <div class="sources-title">联网搜索来源</div>
          <ul class="sources-list">
            <li v-for="(chunk, sIndex) in msg.ragTrace.web_sources" :key="sIndex" class="source-item">
              <div class="source-title-line">
                <a
                  v-if="chunk.url"
              ...
              <div class="source-meta-line">
                <span class="source-page">搜索名次：#{{ chunk.rrf_rank || (sIndex + 1) }}</span>
