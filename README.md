...
- 聊天
  - `POST /chat`：聊天（非流式），入参 `message`、`session_id`、`images`（可选，图片 data URL 列表，支持多模态读图）。
...
- 会话（用户隔离）
  ...
  - `GET /sessions/{session_id}/todos`：读取当前用户某会话的任务清单。
  - `PATCH /sessions/{session_id}/todos`：更新（覆盖）当前用户某会话的任务清单，入参 `todos`。
- 文档（管理员权限）
...
> 以下为本次新增/变更接口的请求与响应结构。所有接口均需在请求头携带 `Authorization: Bearer <token>`。

### POST /chat 与 POST /chat/stream

聊天接口新增可选的 `images` 字段，支持上传图片由模型读图回复。
...
- **请求体**（`application/json`）：
  ...
  | `session_id` | string | 否 | 会话 ID，默认 `default_session` |
  | `images` | string[] | 否 | 图片 data URL 列表（`data:image/...;base64,...`），单轮最多 4 张，单张 ≤ 5MB |

- **行为说明**：
  - 有图片时，后端构造多模态 `HumanMessage`（`text` + 多个 `image_url` block）交给 LangChain Agent，模型可读取图片内容并回复。
  - 历史消息落库只存纯文本，图片仅在当轮进入 Agent 上下文，不持久化。
  - `/chat/stream` 为 SSE 流式响应，事件类型同既有：`content`、`rag_step`、`trace`、`session_title`、`error`、`[DONE]`。

### GET /sessions/{session_id}/todos

读取当前用户某会话的任务清单。任务由 AI 在每轮对话结束后自动从对话中提取可执行待办，按会话存入 `ChatSession.metadata_json.todos`。
...
- **响应**（`application/json`）：
  ...
  {
    "todos": [
      { "id": "8f...e2", "text": "周三前交季度报告", "done": false, "created_at": "2026-06-18T10:00:00" },
      { "id": "a1...77", "text": "预约周五的牙医", "done": true, "created_at": "2026-06-18T10:05:00" }
    ]
  }
  ```

### PATCH /sessions/{session_id}/todos

整体覆盖更新当前用户某会话的任务清单，用于前端勾选完成、删除、清空已完成等操作。
...
- **请求体**（`application/json`）：
  ...
  {
    "todos": [
      { "id": "8f...e2", "text": "周三前交季度报告", "done": true, "created_at": "2026-06-18T10:00:00" }
    ]
  }
  ```

- **响应**：同 `GET /sessions/{session_id}/todos`，返回更新后的完整清单。
- **行为说明**：后端用新 dict 赋值 `metadata_json` 以确保 SQLAlchemy 检测到 JSON 字段变更并正确持久化。
...
### 2026-06-23 联网搜索与输入区整理
- 新增前端“搜索”开关：开启后会随 `/chat`、`/chat/stream` 请求传递 `web_search_enabled`，且不会在单轮回答结束后自动关闭；新建会话或清空会话时重置。
- 后端新增 Open WebSearch fallback：搜索工具在启用后仍优先执行知识库 RAG，只有知识库结果不足时才补充联网搜索，并在 trace 与引用区展示 web 来源、搜索词和抓取页面。
- 新增 `web-search` Docker Compose 服务与 `WEB_SEARCH_*` 环境变量，默认本地地址为 `http://127.0.0.1:3210`。
- 修复流式回答中内部结构化评分结果泄露的问题，仅将顶层 Agent 最终回答 token 推送给前端。
- 相关性评分解析兼容 `binary_score` 与 `score` 两种字段，降低不同 OpenAI 兼容模型的结构化输出差异影响。
- 前端输入区移除无实际功能的“思考”占位按钮，保留图片、任务、搜索与发送/终止操作。

### 2026-06-18 图片多模态对话与任务清单
- 聊天接口（`/chat`、`/chat/stream`）新增 `images` 可选字段，支持上传图片（base64 data URL），后端构造多模态 `HumanMessage` 交由模型读图回复；历史消息落库仍存纯文本，图片仅在当轮进入 Agent 上下文。
- 新增任务清单功能：每轮对话结束后用 `fast_model` 自动从对话中提取可执行待办，按会话存入 `ChatSession.metadata_json.todos`，文本去重追加并保留已有项 done 状态。
- 新增 `GET / PATCH /sessions/{session_id}/todos` 接口读写任务清单；修复 `update_todos` 原地修改 JSON 字段导致删除/勾选不持久化的 bug。
- 前端聊天框新增图片上传（预览/移除/数量与大小校验）、任务清单视图（勾选/删除/清空已完成）及侧栏导航。
