...
class GradeDocuments(BaseModel):
    ...
    score: Optional[str] = Field(
        default=None,
        description="Alternative relevance score field: 'yes' if relevant, or 'no' if not relevant",
    )

    def normalized_score(self) -> str:
        return (self.binary_score or self.score or "").strip().lower()
...
def _normalize_grade_value(value) -> str:
    score = str(value or "").strip().lower()
    if score in ("yes", "true", "relevant", "1"):
        return "yes"
    if score in ("no", "false", "irrelevant", "0"):
        return "no"
    return ""


def extract_grade_score(content) -> str:
    """从结构化或普通模型输出里提取 yes/no，兼容 binary_score 与 score 字段。"""
    if isinstance(content, GradeDocuments):
        return content.normalized_score()
    if isinstance(content, dict):
        return _normalize_grade_value(content.get("binary_score") or content.get("score"))
    ...
    for idx, char in enumerate(text):
        ...
        for item in items:
            if isinstance(item, dict):
                score = _normalize_grade_value(item.get("binary_score") or item.get("score"))
                if score:
                    return score

    match = re.search(r'"(?:binary_)?score"\s*:\s*"(yes|no|true|false|relevant|irrelevant|1|0)"', text, re.IGNORECASE)
    if match:
        return _normalize_grade_value(match.group(1))

    if re.search(r"\byes\b|相关|足够|relevant", text, re.IGNORECASE):
        return "yes"
    if re.search(r"\bno\b|不相关|不足|irrelevant", text, re.IGNORECASE):
        return "no"
    return ""
...
def grade_documents_node(state: RAGState) -> RAGState:
    ...
    prompt = (
        GRADE_PROMPT.format(question=question, context=context)
        ...
        if router:
            ...
            try:
                decision = structured_invoke(
                    router, RewriteStrategy, [{"role": "user", "content": prompt}]
                )
...
def classify_complexity(state: RAGState) -> RAGState:
    ...
    try:
        result = structured_invoke(
            model, ComplexityResult, [{"role": "user", "content": prompt}]
        )
...
def decompose_question(state: RAGState) -> RAGState:
    ...
    try:
        result = structured_invoke(
            model, SubQuestions, [{"role": "user", "content": prompt}]
        )
...
@traceable(name="run_rag_graph", metadata={"graph_entry": "classify_complexity"})
